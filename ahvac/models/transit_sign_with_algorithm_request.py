...
class TransitSignWithAlgorithmRequest(BaseModel):
    """
    ...
    batch_input: Optional[List[Dict[str, Any]]] = Field(default=None, description="Specifies a list of items for processing. When this parameter is set, any supplied 'input' or 'context' parameters will be ignored. Responses are returned in the 'batch_results' array component of the 'data' element of the response. Any batch output will preserve the order of the batch input")
    ...
    hash_algorithm: Optional[StrictStr] = Field(default='sha2-256', description="Hash algorithm to use (POST body parameter). Valid values are: * sha1 * sha2-224 * sha2-256 * sha2-384 * sha2-512 * sha3-224 * sha3-256 * sha3-384 * sha3-512 * none Defaults to \"sha2-256\". Not valid for all key types, including ed25519. Using none requires setting prehashed=true and signature_algorithm=pkcs1v15, yielding a PKCSv1_5_NoOID instead of the usual PKCSv1_5_DERnull signature.")
    input: Optional[StrictStr] = Field(default=None, description="The base64-encoded input data")
    key_version: Optional[StrictInt] = Field(default=None, description="The version of the key to use for signing. Must be 0 (for latest) or a value greater than or equal to the min_encryption_version configured on the key.")
    marshaling_algorithm: Optional[StrictStr] = Field(default='asn1', description="The method by which to marshal the signature. The default is 'asn1' which is used by openssl and X.509. It can also be set to 'jws' which is used for JWT signatures; setting it to this will also cause the encoding of the signature to be url-safe base64 instead of using standard base64 encoding. Currently only valid for ECDSA P-256 key types\".")
    ...
    signature_algorithm: Optional[StrictStr] = Field(default=None, description="The signature algorithm to use for signing. Currently only applies to RSA key types. Options are 'pss' or 'pkcs1v15'. Defaults to 'pss'")
    __properties: ClassVar[List[str]] = ["algorithm", "batch_input", "context", "hash_algorithm", "input", "key_version", "marshaling_algorithm", "prehashed", "salt_length", "signature_algorithm"]
