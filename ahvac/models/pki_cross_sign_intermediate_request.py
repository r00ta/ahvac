...
class PkiCrossSignIntermediateRequest(BaseModel):
    """
    PkiCrossSignIntermediateRequest
    """ # noqa: E501
    add_basic_constraints: Optional[StrictBool] = Field(default=None, description="Whether to add a Basic Constraints extension with CA: true. Only needed as a workaround in some compatibility scenarios with Active Directory Certificate Services.")
    ...
    exported: Optional[StrictStr] = Field(default=None, description="Must be \"internal\", \"exported\" or \"kms\". If set to \"exported\", the generated private key will be returned. This is your *only* chance to retrieve the private key!")
    ...
    __properties: ClassVar[List[str]] = ["add_basic_constraints", "alt_names", "common_name", "country", "exclude_cn_from_sans", "exported", "format", "ip_sans", "key_bits", "key_name", "key_ref", "key_type", "locality", "managed_key_id", "managed_key_name", "not_after", "not_before_duration", "organization", "other_sans", "ou", "postal_code", "private_key_format", "province", "serial_number", "signature_bits", "street_address", "ttl", "uri_sans"]

    @field_validator('exported')
    def exported_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['internal', 'external', 'kms']):
            raise ValueError("must be one of enum values ('internal', 'external', 'kms')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "add_basic_constraints": obj.get("add_basic_constraints"),
